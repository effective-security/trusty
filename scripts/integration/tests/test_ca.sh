...
#/opt/trusty/bin/trustyctl $CA_FLAGS -s $TRUSTY_CA_1 --timeout 3 ca issuer --label SHAKEN_G1_CA
...
/opt/trusty/bin/hsm-tool --cfg=inmem csr create --plain-key --key-label=ci_test* --csr-profile=/opt/trusty/etc/dev/csr_profile/trusty_server.yaml --output /tmp/csr
