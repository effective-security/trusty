...
until PGPASSWORD=$POSTGRES_PWD psql -h $POSTGRES_HOST -p $POSTGRES_PORT -U $POSTGRES_USER -lqt | cut -d \| -f 1 | grep -qw cadb; do
  >&2 echo "cadb is unavailable $POSTGRES_HOST:$POSTGRES_PORT - creating"
  >&2 PGPASSWORD=$POSTGRES_PWD psql -h $POSTGRES_HOST -p $POSTGRES_PORT -U $POSTGRES_USER -a -f /opt/trusty/sql/cadb/create.sql
